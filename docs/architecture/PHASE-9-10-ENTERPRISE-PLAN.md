...
> **IMPLEMENTED — Phase 10c code-complete + verified live (2026-07-12).** Deviations/corrections that
> supersede the sketches below (everything else landed as written; every metric name in the dashboards
> was verified against live endpoints before being committed):
>
> - **Chart pins (2026-07-12):** kube-prometheus-stack **87.15.1**, loki **7.0.0**, alloy **1.10.1**,
>   tempo **2.2.3** from **grafana-community** (the `grafana/tempo` chart is deprecated — repository
>   migration; updates land only in grafana-community/helm-charts after 2026-01-30),
>   postgres-exporter **8.1.1**. Defaults in `lib.sh`, overrides in `.env`.
> - **monitoring ns PSS = `enforce: privileged` + `warn/audit: baseline`, NOT the sketched `baseline`
>   enforce** — baseline FORBIDS hostNetwork/hostPID/hostPath, which node-exporter requires; an
>   enforce=baseline namespace rejects its pods at admission (the install's own PSS warning confirmed
>   it live). warn=baseline keeps drift visible for everything else.
> - **Exemplars needed backend changes on BOTH axes** (`ObservabilityExtensions` + CPM): current OTel
>   .NET defaults the exemplar filter to **AlwaysOff** — this block's "TraceBasedExemplarFilter is the
>   default" is stale — so `.SetExemplarFilter(ExemplarFilterType.TraceBased)` was added; AND the
>   Prometheus exporter only gained OpenMetrics exemplar exposition in **1.16.0-beta.1** (PR 7222), so
>   the whole OTel family moved 1.10 → 1.16 (exporter pins core ≥1.16; also clears the 1.10 NU1902
>   advisories; Api.Tests + ObservabilityFlowTests re-run green). Verified live: exemplar → Tempo trace.
> - **Tempo's HTTP API is port 3200, not 3100** (3100 is Loki) — Grafana datasource corrected.
> - **MinIO metric corrections (verified against RELEASE.2025-09-07):** bucket usage/objects live on a
>   SECOND endpoint `/minio/v2/metrics/bucket` (ServiceMonitor scrapes both); TTFB is
>   `minio_s3_requests_ttfb_seconds_distribution` (le-labeled, NO `_bucket` suffix).
> - **.NET runtime metric names are `dotnet_*`** (`dotnet_gc_collections_total{gc_heap_generation}`,
>   `dotnet_gc_pause_time_seconds_total`, …) — Runtime instrumentation 1.10 forwards the built-in
>   `System.Runtime` meter; the sketched `process_runtime_dotnet_*` names don't exist on /metrics.
> - **minikube's cAdvisor (docker driver) exports POD-SLICE series only** — no `container` label at all,
>   so every sketched `container!=""` filter matches nothing. Dashboards/alerts use `container=""`
>   (selects the pod slice on full cAdvisor too — double-count-safe both ways); PodMemoryNearLimit is
>   pod-level vs summed container limits. `kubelet_volume_stats_*` doesn't exist on hostpath PVs —
>   the PVC panel/alert are kept but documented as live-empty on minikube.
> - **Loki chart + `persistence: false` mounts NOTHING at /var/loki** → crashloop on the read-only
>   rootfs; values add an explicit emptyDir (the "disposable storage" actually intended). The
>   SimpleScalable replicas (3/3/3 defaults) must also be zeroed explicitly in chart 7.x.
> - **Dashboards: 8, not 7** — the plan's seven plus `errors-and-problems.json` (recent-errors Loki
>   table, unhandled-by-category, 401/403-vs-422 rejection split, failing-routes table, background-loop
>   tick age, poison trend). Dashboards are plain JSON wrapped into ConfigMaps by monitoring-up.sh
>   (editable/UI-importable); `QUERIES.md` (generated by `extract-queries.py`) is the greppable index
>   for 9c. Alert rules: the 13 sketched + `BackgroundLoopStale` + `UnhandledErrorsPresent` = 15.
> - **Backend ServiceMonitor needs `honorLabels: true` (found live):** the operator attaches
>   `service=<Service name>` as a target label, colliding with the Forum Meter's own `service` tag —
>   without it `forum_hosted_service_tick_age_seconds`'s loop names get renamed to
>   `exported_service`, the dead-loop panel flattens to one series and BackgroundLoopStale
>   false-fires on the sweeper's normal 15-min tick age (observed firing, resolved by the fix).
> - **kps 87.x kubelet defaults already include cAdvisor+https+insecureSkipVerify** and the
>   `bearerTokenFile` values key no longer exists (operator injects the token) — settings kept
>   explicit, stale key dropped. pg-exporter gets `fullnameOverride: postgres-exporter` so the job
>   label is sane; its credentials are the app ns `postgres-credentials` secret mirrored into
>   `monitoring` by monitoring-up.sh.
> - **No `port-forward.sh`** (the §11 inventory sketch) — 10b's `dev-tunnels.sh` is the single access
>   mechanism; it gained grafana (13001) / prometheus (19090) entries against the VERIFIED Service
>   names `monitoring-grafana` / `monitoring-kube-prometheus-prometheus`, opened only when the ns exists.
> - **Verified on MINIKUBE_MEMORY=8192** (this machine's WSL still grants 10GB — §1's 10240 target
>   needs the .wslconfig bump first): all targets UP, whole-stack actual usage ≈ **944Mi** RSS /
>   ~54m CPU, node at 39% memory with app + monitoring both running. Fits even the reduced VM.
...
> **IMPLEMENTED (partially) — Phase 10d code changes landed + verified live (2026-07-15), measured
> before/after runs still owed to 9c.** This block ran ahead of 9c because a live investigation session
> (interactive lag → per-IP 429 storms) had already produced field data; the code-level items below are
> in and verified on the cluster, but the DoD's `bench-run.sh demo` before/after archive CANNOT exist
> until 9c lands — re-run this block's step 6 checklist then. What landed and what the live audit found:
>
> - **Redis verdict CONFIRMED against live data, decision unchanged: no Redis.** Every candidate was
>   re-checked with the 10c stack instead of by reasoning alone: permission checks and all hot reads are
>   sub-2 ms single/double Npgsql spans in Tempo (feed = exists-check + view read; detail/search/batch =
>   one span each) — there is nothing for a cache tier to win. The only honest candidate (WS ticket
>   replay cache) keeps its "No by default" from the table below. Nothing found in the audit moves any
>   row's verdict.
> - **#1 Max Auto Prepare:** `;Max Auto Prepare=20;Auto Prepare Min Usages=2` appended in deploy.sh's
>   generated CONNECTION_STRING + `k8s/postgres/secret.example.yaml` + patched into the live secret
>   (deploy.sh never overwrites existing secrets — remember to patch manually on clusters that predate
>   this, as done here).
> - **#2 gzip at ingress:** landed in `setup-minikube.sh` (controller-scoped config belongs with the
>   addon, not deploy.sh) as an idempotent merge-patch of the `ingress-nginx-controller` ConfigMap:
>   `use-gzip` + `gzip-types` (json/problem+json/js/css/html/svg) + `gzip-min-length: 1024`; the
>   controller hot-reloads. Verified live: `content-encoding: gzip` on API JSON through
>   `https://forum.local`.
> - **#3 min-threads:** `DOTNET_ThreadPool_MinThreads: "64"` in `k8s/backend/configmap.yaml`. Live
>   Tempo justification recorded in its comment: `POST /api/identity/login` ≈ 409 ms of which <5 ms is
>   SQL — pure Argon2id CPU on a pod whose 750m limit means the runtime sees ONE processor. NOTE: the
>   idle thread-count gauge does NOT show 64 — SetMinThreads sets an injection floor, it pre-creates
>   nothing; don't "verify" it that way (only a burst shows it).
> - **#4 GC: verified, deliberately NO knob added.** Working set ≈300 Mi / committed ≈100 Mi under the
>   512 Mi limit; .NET's container-aware default heap cap (75 % of cgroup limit) already equals the
>   sketched `DOTNET_GCHeapHardLimitPercent=75` — adding it would be config noise claiming to change
>   something.
> - **#5 N+1 sweep: CLEAN, verified in Tempo span-by-span** (the design goal "reads via SQL views +
>   keyset, no N+1" is true in fact, not just intent). Per-route Npgsql span counts: feed 2, thread
>   detail 1, comments 2, reactions single/batch 1, search 1, files-by-target 1, categories 1 — all
>   sub-2 ms. The REAL N+1 was client-side, over HTTP: 24 h of interactive traffic showed
>   `GET /api/files` (avatar/icon per card) as the TOP API route (227 calls vs 65 feed loads) and 179
>   single reaction GETs duplicating 77 batch GETs. Fixed in the SPA, not the backend: central
>   `lib/api/stale-times.ts` freshness tiers (realtime-covered 5 min — push-invalidation is the
>   freshness mechanism while subscribed, staleTime only governs back-nav; presigned-file queries 5 min,
>   hard-bounded by Files:DownloadUrlTtlMinutes=15; reference 60 s; search 30 s), `useReactionBatch`
>   write-through into per-target cache entries + `covered` ReactionButtons that never self-fetch, and
>   the reconnect resync narrowed from "every active query" to the push-covered key families
>   (threads/comments/reactions) — a reconnect carries no information about anything else.
> - **WS hub hardening (found by the audit, not in the sketch):** the dispatcher sends sequentially per
>   event with no send deadline, so ONE peer that stopped reading (full TCP window) would head-of-line
>   block pushes to every subscriber on the replica, indefinitely. `RealtimeConnection.TrySendAsync` now
>   aborts a socket whose send (or gate wait) exceeds 5 s — costs the hub one connection instead of the
>   replica's whole push path (client reconnect-resyncs per ADR 0010). Unit-tested with a stalling
>   fake socket. The per-subscriber permission checks in the dispatcher (the suspected "N+1") were
>   MEASURED as a non-issue and left alone: private-category events only, one `effective_perm_cache` PK
>   lookup per subscriber, µs each at forum-scale subscriber counts.
> - **Observability corrections while verifying:** `overview.json`'s aggregate p95 and `app-red.json`'s
>   per-route p95 still included `/api/realtime/ws` (socket-lifetime samples, reported only at close —
>   same gap the session had already fixed in app-red's three aggregate panels); both now exclude it,
>   QUERIES.md regenerated. Tempo itself OOM'd (exit 137, 512 Mi limit) under a burst of TraceQL
>   searches during this audit — fine for dashboards/exemplar hops, but pace `/api/search` scripting
>   against it (or raise its limit when the .wslconfig 12 GB / MINIKUBE_MEMORY=10240 bump happens).
> - **Deliberately NOT done, with reasons:** React-Query persister (localStorage/IndexedDB) — cached
>   entries embed presigned URLs that expire in 15 min, so restoring them across reloads trades one
>   cold-load burst for broken images and stale-forever content; "LOAD MORE ↓" stays a button, not an
>   IntersectionObserver — it is the design language's deliberate pagination affordance and auto-fetch
>   would spend rate-limiter budget on scroll momentum; categories/tags staleTime stays modest (60 s) —
>   no `CategoryCreated` realtime event exists (Phase 7 decision), and adding one for a rarely-changing
>   entity is not worth new bus surface; per-IP rate limiter stays per-replica per this block's table
>   (the ×replicas multiplier note belongs in the thesis).

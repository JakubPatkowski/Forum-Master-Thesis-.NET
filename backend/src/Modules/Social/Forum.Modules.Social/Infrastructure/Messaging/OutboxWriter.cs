...
namespace Forum.Modules.Social.Infrastructure.Messaging;

/// <summary>
/// Writes integration events as <c>outbox_messages</c> rows on the Social DbContext, so they commit in the
/// same transaction as the state change. The relay publishes and stamps them; the correlation id rides along so
/// the consumer side logs under the originating request's id.
/// </summary>
internal sealed class OutboxWriter : IOutboxWriter
{
    ...
    public OutboxWriter(SocialDbContext db, ICorrelationContext correlation)
    {
        _db = db;

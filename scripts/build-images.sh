...
# path (see frontend/Dockerfile). Default: http://$INGRESS_HOST; override for a local
# smoke test with NEXT_PUBLIC_API_URL=http://localhost:8080.
...
api_url="${NEXT_PUBLIC_API_URL:-http://$INGRESS_HOST}"
case "$api_url" in
  */api | */api/)
